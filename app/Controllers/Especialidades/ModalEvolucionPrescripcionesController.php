...
/**
 * Modal de Evolución y Prescripciones - Versión Corregida
...
class ModalEvolucionPrescripcionesController extends BaseController
{
    ...
    public function modalEvolucionPrescripciones()
    {
        ...
        try {
            ...
            $are_codigo_raw = $input['are_codigo'] ?? '';
            $ate_codigo = $input['ate_codigo'] ?? '';
            $esp_codigo = $input['esp_codigo'] ?? '';
            $pac_datos = $input['pac_datos'] ?? [];

            // Limpiar are_codigo si viene como array
            $are_codigo = '';
            if (is_array($are_codigo_raw)) {
                $are_codigo = !empty($are_codigo_raw) ? $are_codigo_raw[0] : '';
            } else {
                $are_codigo = $are_codigo_raw;
            }

            // Convertir a string si es necesario
            $are_codigo = (string) $are_codigo;
            ...
            // Si are_codigo está vacío, intentar obtenerlo de ate_codigo
            if (empty($are_codigo)) {
                ...
                if ($areaAtencion) {
                    $are_codigo = (string) $areaAtencion['are_codigo'];
                }
            }
            ...
            // Generar número de hoja para esta atención
            ...
            return view('especialidades/modal_evolucion_prescripciones', $data);
        } catch (\Exception $e) {
    ...
    public function obtenerEvolucionesConUsuario()
    {
        ...
        try {
            ...
            $evoluciones = $this->evolucionModel->obtenerConUsuario($ate_codigo);
            ...
            return $this->response->setJSON([
                'success' => false,
                'error' => 'Error al obtener evoluciones con usuario'
            ]);
        }
    }
    ...
    public function guardarEvolucionPrescripciones()
    {
        ...
        try {
            ...
            if (!$input || !isset($input['ate_codigo'])) {
            ...
            // Guardar usando el modelo (sin datos adicionales de establecimiento)
            $resultado = $this->evolucionModel->guardarMultiples($ate_codigo, $filas);

            if ($resultado['success']) {
                $mensaje = 'Evolución y prescripciones guardadas correctamente';
                if ($resultado['registros_actualizados'] > 0) {
                    $mensaje .= " - {$resultado['registros_actualizados']} actualizados";
                }
                if ($resultado['registros_insertados'] > 0) {
                    $mensaje .= " - {$resultado['registros_insertados']} nuevos";
                }
                if ($resultado['registros_eliminados'] > 0) {
                    $mensaje .= " - {$resultado['registros_eliminados']} eliminados";
                }

                return $this->response->setJSON([
                    'success' => true,
                    'message' => $mensaje,
                    'registros_guardados' => $resultado['total_procesados'] ?? 0,
                    'detalles' => [
                        'actualizados' => $resultado['registros_actualizados'] ?? 0,
                        'insertados' => $resultado['registros_insertados'] ?? 0,
                        'eliminados' => $resultado['registros_eliminados'] ?? 0
                    ]
                ]);
            } else {
                return $this->response->setJSON([
                    'success' => false,
                    'error' => $resultado['error'] ?? 'Error al guardar'
                ]);
            }
        } catch (\Exception $e) {
            return $this->response->setJSON([
    ...
    public function contarEvolucionPrescripciones()
    {
        ...
        try {
            ...
            $count = $this->evolucionModel->contarPorAtencion($ate_codigo);
