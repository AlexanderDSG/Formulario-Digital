            ...
            <div class="relative py-3 sm:max-w-xl sm:mx-auto">
                <div class="relative px-4 py-10 bg-white mx-8 md:mx-0 shadow rounded-3xl sm:p-10">
                    ...
                    <div class="max-w-md mx-auto">
                        ...
                        <!-- Formulario -->
                        <form action="<?= base_url('administrador/usuarios/actualizar/' . $usuario['usu_id']) ?>" method="POST" class="space-y-4">
                            <?= csrf_field() ?>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" name="usu_nombre"
                                        ...
                                        value="<?= esc($usuario['usu_nombre']) ?>" required>
                                ...
                                <div>
                                    ...
                                    <input type="text" name="usu_apellido"
                                        class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                        value="<?= esc($usuario['usu_apellido']) ?>" required>
                            ...
                            <div>
                                ...
                                <input type="text" name="usu_nro_documento"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                    value="<?= esc($usuario['usu_nro_documento'] ?? '') ?>"
                                    placeholder="Ingrese cédula, pasaporte u otro documento" required>
                                ...
                                <?php endif; ?>
                                <?php if (empty($usuario['usu_nro_documento'])): ?>
                                    <p class="text-orange-600 text-sm mt-1">
                                        <i class="fas fa-exclamation-triangle mr-1"></i>
                                        Este usuario no tiene número de documento registrado
                                    </p>
                                <?php endif; ?>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" name="usu_usuario"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                    value="<?= esc($usuario['usu_usuario']) ?>" required>
                            ...
                            <div>
                                ...
                                <select name="rol_id" id="rol_id"
                                    ...
                                    <option value="2" <?= $usuario['rol_id'] == 2 ? 'selected' : '' ?>>Admisionista</option>
                                    <option value="3" <?= $usuario['rol_id'] == 3 ? 'selected' : '' ?>>Enfermera</option>
                                    <option value="4" <?= $usuario['rol_id'] == 4 ? 'selected' : '' ?>>Médico</option>
                                    <option value="5" <?= $usuario['rol_id'] == 5 ? 'selected' : '' ?>>Médico Especialista</option>
                                </select>
                            ...
                            <div id="especialidades-container" class="hidden">
                                ...
                                <select name="especialidades[]" id="especialidades"
                                    ...
                                    <?php
                                    // Crear array de especialidades del usuario
                                    $especialidadesUsuarioIds = [];
                                    if (!empty($especialidades_usuario)) {
                                        foreach ($especialidades_usuario as $esp_usu) {
                                            $especialidadesUsuarioIds[] = $esp_usu['esp_codigo'];
                                        }
                                    }
                                    ?>
                                    <?php foreach ($especialidades as $especialidad): ?>
                                        <?php
                                        $selected = false;
                                        // Si hay old input, usar ese
                                        if (old('especialidades')) {
                                            $selected = is_array(old('especialidades')) && in_array($especialidad['esp_codigo'], old('especialidades'));
                                        } else {
                                            // Si no hay old input, usar las especialidades del usuario
                                            $selected = in_array($especialidad['esp_codigo'], $especialidadesUsuarioIds);
                                        }
                                        ?>
